...
@ccclass('PVPChooseBattleManager')
export class PVPChooseBattleManager extends Component {
    private static singleton: PVPChooseBattleManager;
    public static getInstance(): PVPChooseBattleManager {
        return PVPChooseBattleManager.singleton;
    }
    @property({ type: DinoDetailManager })
    public dinoDetailManager: DinoDetailManager[]= [];
    ...
    start () {
        PVPChooseBattleManager.singleton = this;       
        ...
        this.betList = Common.currentData["betList"]; 
        ...
        this.SetBettingType();
        this.bettingNode.active = true;
        this.findingNode.active = false;
        this.foundNode.active = false;
        this.blockSelectDino(false);
    }
    public SetBettingType()
    {
        for (let index = 0; index < this.betLabel.length; index++) {
            const element = this.betLabel[index];
            element.node.parent.active = this.betList.length > index;
            if(this.betList.length > index)
            element.string = Common.formatPrice(this.betList[index]) + " W-DNL";
        }
    }
    
    public OnBettingChange(event:any, idx:number)
    {
    ...
    {    
        ...
        PVPChooseBattleManager.getInstance().bettingNode.active = false;
        PVPChooseBattleManager.getInstance().findingNode.active = false;
        PVPChooseBattleManager.getInstance().nextBtn.interactable = true;
        PVPChooseBattleManager.getInstance().prevBtn.interactable = true;
        PVPChooseBattleManager.getInstance().foundNode.active = true;
        let value = 5;
        if(result["code"] == 1){
            PVPChooseBattleManager.getInstance().countDownFound.string = value+"s";
            PVPChooseBattleManager.getInstance().schedule(()=>{
                if(value >= 1) {value--; PVPChooseBattleManager.getInstance().countDownFound.string = value + "s";}
            },1)
            PVPChooseBattleManager.getInstance().schedule(()=>{
                PVPChooseBattleManager.getInstance().fightChecking = false;        
                Common.currentData = result["data"];
                director.loadScene("GameplayPVP");
            },5)
        }else{
            PVPChooseBattleManager.getInstance().bettingNode.active = true;
            PVPChooseBattleManager.getInstance().findingNode.active = false;
            PVPChooseBattleManager.getInstance().foundNode.active = false;
            PVPChooseBattleManager.getInstance().unschedule(this.countBySecond);
            window.foundMatch = null;
            PVPChooseBattleManager.getInstance().blockSelectDino(false);
            PVPChooseBattleManager.getInstance().ResetFightButton();
        }
    }
    ...
    public OnFightButtonClicked()
    {
        ...
        finalChoosenData["bet"] = this.betList[this.currentBetIndex];
        finalChoosenData["betType"] = 1;
        finalChoosenData["dinoList"] = new Array();

        for (let index = 0; index < PVPChooseBattleManager.getInstance().listChoosingCard.length; index++) {
            const element = PVPChooseBattleManager.getInstance().listChoosingCard[index];
        ...
        this.bettingNode.active = false;
        this.findingNode.active = true;
        this.foundNode.active = false;
        this.betTxt.string = Common.formatPrice(finalChoosenData["bet"]) + " W-DNL";
        this.countDownNum = 0;
