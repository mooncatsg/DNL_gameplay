...
#region ErrorValues
        ...
        private void OnDrawElementBackgroundCallback(Rect rect, int index, bool isActive, bool isFocused)
        {
            ...
            if (Event.current.type == UnityEngine.EventType.MouseDown && spliterRect.Contains(Event.current.mousePosition))
            {
...
public class MySearchField : SearchField
{
        ...
        GUIStyle emptyCancelButtonStyle)
    {
        ...
        if (Event.current.type == UnityEngine.EventType.MouseDown && ContextMenuRect.Contains(Event.current.mousePosition))
        {
